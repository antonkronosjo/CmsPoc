...
public sealed class PolymorphicQueryTests : IDisposable
{
    ...
    {
        var news = _fixture.Repository.Create(new NewsContent { Name = "HEJ", Heading = "Hello", Body = "World", Color = "red" }, "en");
        var @event = _fixture.Repository.Create(new EventContent { Name = "HEJ", Title = "T", Description = "D", StartDate = DateTime.UtcNow }, "en");
        _fixture.Repository.Create(new NewsContent { Name = "SOMETHING_ELSE", Heading = "X", Body = "Y", Color = "green" }, "en");

        var results = _fixture.Repository.Query<Content>("en").Where(x => x.Name == "HEJ").ToList();

        Assert.Equal(2, results.Count);
        Assert.Contains(results, c => c is NewsContent nc && nc.Id == news.Id && nc.Color == "red");
    ...
    {
        var news = _fixture.Repository.Create(new NewsContent { Name = "A", Heading = "H", Body = "B", Color = "red" }, "en");
