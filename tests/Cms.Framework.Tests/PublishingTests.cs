...
public sealed class PublishingTests : IDisposable
{
    ...
    {
        var created = _fixture.Repository.Create(new NewsContent { Name = "A", Heading = "H", Body = "B", RelatedContent = new ContentReference(1, "NewsContent") }, "en");
    ...
    {
        var news = _fixture.Repository.Create(new NewsContent { Name = "N", Heading = "H", Body = "B", RelatedContent = new ContentReference(1, "NewsContent") }, "en");
        var @event = _fixture.Repository.Create(new EventContent { Name = "E", Title = "T", Description = "D", StartDate = DateTime.UtcNow }, "en");
        var unpublishedNews = _fixture.Repository.Create(new NewsContent { Name = "N2", Heading = "H2", Body = "B2", RelatedContent = new ContentReference(2, "NewsContent") }, "en");
    ...
    public void Search_and_GetSummary_show_the_published_version_when_one_exists_and_fall_back_to_latest_otherwise()
    {
        ...
        Assert.Equal(new ContentReference(2, "NewsContent"), draftSummary!.Properties["RelatedContent"]);

        var draftSearch = _fixture.Editing.Search(query: null, "en", contentTypeName: null, page: 1, pageSize: 20);
        Assert.Equal(new ContentReference(2, "NewsContent"), draftSearch.Items.Single(i => i.Id == created.Id).Properties["RelatedContent"]);

        _fixture.Editing.Publish(created.Id, 1, startPublish: null, stopPublish: null);

        var publishedSummary = _fixture.Editing.GetSummary(created.Id, "en");
        Assert.Equal(new ContentReference(1, "NewsContent"), publishedSummary!.Properties["RelatedContent"]);

        var publishedSearch = _fixture.Editing.Search(query: null, "en", contentTypeName: null, page: 1, pageSize: 20);
        Assert.Equal(new ContentReference(1, "NewsContent"), publishedSearch.Items.Single(i => i.Id == created.Id).Properties["RelatedContent"]);

        var publishedOnlySearch = _fixture.Editing.Search(query: null, "en", contentTypeName: null, page: 1, pageSize: 20, publishedOnly: true);
        Assert.Equal(new ContentReference(1, "NewsContent"), publishedOnlySearch.Items.Single(i => i.Id == created.Id).Properties["RelatedContent"]);
    }
