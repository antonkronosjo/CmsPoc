...
/// interface (version creation, per-language branching, live resolution of
/// shared values, joins) is handled internally.
/// </summary>
public interface IContentRepository
{
    /// <summary>Creates a new content instance (root + version 1 of the given language branch).</summary>
    T Create<T>(T content, string language) where T : Content;

    /// <summary>
    /// Creates a new, immutable version in one language branch. The branch is
    /// <see cref="Content.Language"/> on the passed-in instance; each language
    /// has its own version numbering, history and publish state, so other
    /// languages are untouched. Shared (non-culture-specific) values can only
    /// be changed through the master language branch; saving any other
    /// language never stores them; they are resolved live from the master
    /// branch's published (or, with nothing published, latest) version instead.
    /// </summary>
