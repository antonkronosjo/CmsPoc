...
public interface IContentRepository
{
    ...
    /// <summary>
    /// Queries the current version of content. When <typeparamref name="T"/>
    /// is exactly <see cref="Content"/>, this queries across every
    /// registered content type and preserves each result's concrete runtime
    /// type; predicates are then restricted to members declared on
    /// <see cref="Content"/> itself. When <typeparamref name="T"/> is a
    /// concrete content type, arbitrary predicates over its own flat
    /// properties are supported.
    /// </summary>
    IContentQuery<T> Query<T>(string? language = null) where T : Content;
